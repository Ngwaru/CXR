...
model = models.load_model("cxr_normal_tb_vgg19_324_model.keras")

model_xray_or_not = models.load_model("cxr_or_not_vgg16_model.keras")

def process_image(path_to_image: str, img_size: tuple)->Any:
    """
    ...
    img_two = img_one.resize(img_size)
...
def is_xray_func(model: Any, path_to_image: str) -> bool:
    ...
    Returns:
        bool: True
    """
    img_data_normalized = process_image(path_to_image, (200,200))
...
def predict_image(model: Any, path_to_image: str)->tuple[List[List], str]:
    ...
    if is_xray:
        img_data_normalized = process_image(path_to_image, (324,324))
...
if st.button("Process Uploaded Picture"):
    ...
    grad_cam = Grad_CAM_class(model, "block5_conv4", "temp_img.png", classes_names, (324,324))
